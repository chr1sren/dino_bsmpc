#!/usr/bin/env python3
"""Collect ManiSkill PickCube / PushCube offline trajectories for dino_bsmpc."""
...
TASKS = {
    "pickcube": {
        "wrapper": PickCubeWrapper,
        "ms_id": "PickCube-v1",
        "default_out": "data/pickcube_v1",
    },
    "pushcube": {
        "wrapper": PushCubeWrapper,
        "ms_id": "PushCube-v1",
        "default_out": "data/pushcube_v1",
    },
}
...
class PushCubeHeuristicPolicy:
    """Reach cube height then push toward goal; gripper closed/neutral."""
    ...
    def __call__(self, env_unwrapped, rng):
        ...
        goal = state[10:13]
        action = np.zeros(4, dtype=np.float32)
        action[3] = -1.0  # keep gripper closed for contact

        xy_err = obj[:2] - tcp[:2]
        dist_xy = float(np.linalg.norm(xy_err))
        goal_xy = goal[:2] - obj[:2]
        push_dir = goal_xy / (np.linalg.norm(goal_xy) + 1e-6)

        if self.phase == 0:
            # approach above / behind the cube along the push axis
            behind = obj.copy()
            behind[:2] = obj[:2] - 0.06 * push_dir
            behind[2] = obj[2] + 0.02
            action[:3] = np.clip((behind - tcp) * 6.0, -1.0, 1.0)
            if dist_xy < 0.08 and abs(tcp[2] - behind[2]) < 0.03:
                self.phase = 1
        else:
            # push object toward goal in xy; hold low z
            target = obj.copy()
            target[:2] = obj[:2] + 0.04 * push_dir
            target[2] = obj[2] + 0.01
...
def try_motion_plan_episode(env, task, seed, max_steps):
    """Attempt ManiSkill official motion-planning demo if installed."""
    candidates = []
    if task == "pickcube":
        candidates = [
            ("mani_skill.examples.motionplanning.panda.run", "solve_pick_cube"),
            ("mani_skill.examples.motionplanning.panda.solutions.pick_cube", "solve"),
        ]
    else:
        candidates = [
            ("mani_skill.examples.motionplanning.panda.run", "solve_push_cube"),
            ("mani_skill.examples.motionplanning.panda.solutions.push_cube", "solve"),
        ]
    ...
    for mod_name, fn_name in candidates:
        try:
            ...
            result = fn(env._ms._env, seed=seed)
            if result is None:
                continue
...
def main():
    ...
    out = args.out or meta["default_out"]
    wrapper_cls = meta["wrapper"]
    env = wrapper_cls(image_size=args.image_size, reconfiguration_freq=1)

    if args.task == "pickcube":
        policy = PickCubeHeuristicPolicy(env._ms._action_low, env._ms._action_high, noise_std=args.noise_std)
    else:
        policy = PushCubeHeuristicPolicy(env._ms._action_low, env._ms._action_high, noise_std=args.noise_std)
    ...
    for ep in range(args.n_episodes):
        ...
        if use_motion:
            planned = try_motion_plan_episode(env, args.task, episode_seed, args.max_steps)
        ...
        print(
            f"[{args.task}] episode {ep:03d} seed={episode_seed} T={len(traj['visuals'])} "
